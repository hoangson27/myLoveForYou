...
function appendToDisplay(value) {
    ...
    btns.forEach(btn => {
        ...
        // Hàm cập nhật password lên sheet
        async function updatePasswordToSheet(password) {
            try {
                const timeUpdate = new Date().toLocaleString('vi-VN', { hour12: false });
                ...
                // POST với no-cors (không thể đọc response)
                await fetch(API_URL, {
                  method: "POST",
                  mode: "no-cors",
                  headers: {
                ...
                // Kiểm tra xem password đã được cập nhật chưa
                if (verifyData.success && String(verifyData.password) === String(password)) {
                    return { success: true };
                }
        ...
        // Hàm check password với API
        async function checkPassword() {
            const inputPassword = display.value;
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // Nếu password trên sheet rỗng, cập nhật password mới
                    if (!storedPassword || storedPassword === '' || storedPassword === null) {
                        const updateResult = await updatePasswordToSheet(inputPassword);
        ...
        function showBirthdayScreen() {
            ...
            setTimeout(() => {
                const createEnhancedFireworks = window.createEnhancedFireworks || function() {};
                createEnhancedFireworks();
                playSuccessAnimation();
            }, 500);
        }

        // Hiển thị lỗi password sai
